...
Бит может быть прочитан в любое время, но его можно изменить только с помощью последовательности записи ключа блокировки.
...
Биты 15-0 `LCKy`: Блокировка конфигурации линии `y` (0-15) порта `x` (A-F).
