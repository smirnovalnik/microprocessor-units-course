...
int main(void)
{
    ...
    /* При разработке учебных программ важно иметь под рукой два документа:
       ...
       к линии 0 порта C (PC0) и подключен катодом к порту, то есть выход
       порта должен подключить катод к земле, чтобы он засветился.
    ...
    */
    ...
    /* Включить светодиод PC0 (LED1).
       ...
       При этом 0 - низкий активный уровень (Vss), 1 - высокий (Vcc) на
       выходе соответсвующей линии порта.
       Однако согласно функциональной схеме, чтобы включить светодиод нужно
       подключить к его катоду землю (Vss), то есть записать в PC0 значение 0.
       Сброс осуществляется с помощью маски 0xFFFFFFFE (все единицы кроме
       последнего бита) и операции побитового И (оператор &).
       */
    GPIOC->ODR = GPIOC->ODR & 0xFFFFFFFE;
