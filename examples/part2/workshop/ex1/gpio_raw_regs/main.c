...
/* Для обращения к периферийным модулям по абсолютным адресам в языке C
   ...
   *((int *)(0x4000)) = 1;
   Но чтобы не писать постоянно эту констукцию использован define:
   REG = 1;
   ...
   Использование знаковой переменной при обращаении к регистрам может создавать потенциальные ошибки
   со старшим битом.
   volatile - указывает компилятору на то, что каждый раз когда в коде будет производится чтение
   по данному адресу необходимо иммено читать эту ячейку, а не использовать копию из регистра центрального
   ...
   После получения определеного опыта рабты с указателями эти конструкции будут казаться элементарными */
