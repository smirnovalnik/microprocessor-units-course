...
;          y = ~x0 * x3 * ~x5
;
;          Аргумент логической функции имеет размер 6 бит и задется
;          переключателями SW1-SW3.
;          Из-за ограничений учебного стенда 6 битовое слово задается
;          в два шага. На первом шаге выставляются младшие 3 бита аргумента
;          на переключателях SW1-SW3 и по нажатии на кнопку SB1 аргументы
;          записываются в память. На втором шаге выставлюятся старшие 3 бита
;          аргумента на тех же переключателях SW1-SW3 и по нажатии на кнопку
;          SB2 записываются в память. Таким образом формируется 6 битовое слово.
;          Последовательность задания младших и старших битов не важна.
;          По нажатии на кнопку SB3 происходит передача аргуметов в памяти
;          в подпрограмму выполняющую логическую функцию Logic_Func через
;          регистр R0. Перед выходом из подпрограммы результат логической
;          сохраняется в регистр R1. Далее результат (1 бит) выводится
;          на светодиод LED1.
;
;          | SW3  SW2  SW1  | SW3  SW2  SW1  |
;          |  x5   x4   x3  |  x2   x1   x0  |   ---->   y = f(X), y -> LED1
;          |       SB2      |       SB1      |    SB3
            ...
            ; R0 - аргументы (6 младших бит)
            ; R1 - возвращаемое значение (самый младший бит)
Logic_Func
            MOVS  R1, #0
            MOVS  R2, #0x08
            TST   R0, R2
...
Main_Loop
            ...
            LDR   R2, [R1]          ; Прочитать состояние порта A (SW1-SW3)
            ...
            MOVS  R4, #0x7          ; Загрузить маску 3-х младших битов
            BICS  R0, R0, R4        ; Очистить 3 младших бита
            ORRS  R0, R0, R3        ; Объединить 3 младщих бита R0 и R3
            B     Main_Loop
...
Not_SB1
            ...
            MOVS  R4, #0x38         ; Загрузить маску 3-х старших битов
            BICS  R0, R0, R4        ; Очистить 3 старших бита
            LSRS  R3, R3, #3        ; Сдвинуть биты в старшую позицию
            ORRS  R0, R0, R3        ; Поместить в R0 3 младщих бита
            B     Main_Loop
...
Not_SB2
            ...
            STR   R1, [R2]          ; Записать в PC0(LED1) результат функции
