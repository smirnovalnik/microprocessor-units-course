...
;          отображается на светодиодах LED8-LED16.
...
;          сохраняется в самый младщий бит регистра R1. Далее результат (1 бит)
;          выводится на светодиод LED1.
;
;          | SW4  SW3  SW2  SW1  | SW4  SW3  SW2  SW1  |
;          |  x7   x6   x5   x4  |  x3   x2   x1   x0  |   ---->   y = f(X), y -> LED1
;          |         SB2         |         SB1         |    SB3
;
...
;          ДОСТУПНА ТОЛЬКО ЗАГРУЗКА ПРОГРАММЫ.
            ...
            ; Результат работы функции 0 или 1 должен помещаться
            ; в самый младший бит регистра R1 для последующего отображения
            ; на светодиоде.
Logic_Func
...
Reset_Handler
            ...
            ; Настроить линии PC0(LED1) и PC8(LED9)-PC15(LED16) на вывод
            LDR   R0, =GPIOC_MODER  ; Загрузить адрес регистра GPIOC_MODER в R0
            LDR   R1, =0x55550001   ; Загрузить код 0x55550001 в R1
            STR   R1, [R0]          ; Сохранить R1 по адресу R0
...
Main_Loop
            ; Вывести на LED9-LED16 регистр R0 (аргументы функции)
